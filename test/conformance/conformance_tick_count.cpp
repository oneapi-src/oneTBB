...
TEST_CASE("Subtraction subsequent timestamp") {
    tbb::tick_count tick_f(tbb::tick_count::now());
    tbb::tick_count tick_s(tbb::tick_count::now());
    while ((tick_s - tick_f).seconds() == 0) {
        tick_s = tbb::tick_count::now();
    }
...
static void WaitForDuration( double duration ) {
    ...
    double sec = 0;
    do {
        sec = (tbb::tick_count::now() - start).seconds();
        CHECK_GE(sec, 0);
    } while (sec < duration);
}
...
TEST_CASE("Arithmetic operators") {
    ...
    using interval_type = tbb::tick_count::interval_t;
    interval_type i = t2 - t1;
    interval_type j = t3 - t2;
    interval_type k = t3 - t1;
    CHECK_EQ(std::is_same<tbb::tick_count::interval_t, decltype(i - j)>::value, true);
    CHECK_EQ(std::is_same<tbb::tick_count::interval_t, decltype(i + j)>::value, true);
...
//! Test resolution of tbb::tick_count::interval_t
//! \brief \ref interface \ref requirement
TEST_CASE("tbb::tick_count::interval_t resolution") {
    ...
    for (int i = -range_value; i <= range_value; ++i) {
        ...
        CHECK_LT(my_time - interval_time, tbb::tick_count::resolution());
    }
}
