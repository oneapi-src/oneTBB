...
* `multifunction_node` converting each input signal into some amount of output messages (zero messages are also allowed) to one or several successors.

But expressing the use-case converting multiple input signals (input stream) into signal output is not straightforward to express using only the existing API.
