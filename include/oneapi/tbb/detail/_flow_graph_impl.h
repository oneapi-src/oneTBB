...
template<typename Task_Body>
void execute_in_graph_arena(graph& g, Task_Body task_body);
void enqueue_in_graph_arena(graph &g, graph_task& arena_task);

class graph;

//! Class for tasks generated by lightweight graph nodes.
template<typename F>
class ligthweight_task : public task {
    const F m_func;
...
public:
    ligthweight_task(const F& f, wait_context& wo)
        : m_func(f)
        , m_wait_ctx(wo) {}

    ligthweight_task(F&& f, wait_context& wo)
        : m_func(std::move(f))
        , m_wait_ctx(wo) {}
};
...
private:
    ...
    template<typename Task_Body>
    friend void execute_in_graph_arena(graph& g, Task_Body task_body);
...
//! Spawns a task inside graph arena
template<typename Task_Body>
inline void execute_in_graph_arena(graph& g, Task_Body task_body) {
    if (is_graph_active(g)) {
        __TBB_ASSERT(g.my_task_arena && g.my_task_arena->is_active(), NULL);
        wait_context wait_ctx{ 1 };
        ligthweight_task<decltype(task_body)> task(task_body, wait_ctx);
        task_group_context w_ctx;
        execute_and_wait(task, *g.my_context, wait_ctx, w_ctx);
    }
}
