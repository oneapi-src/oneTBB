...
namespace interface6 {
    ...
    namespace internal {
            ...
            void table_elementwise_copy( const ets_base& other,
                ...
                for( array* r=other.my_root; r; r=r->next ) {
                    for( size_t i=0; i<r->size(); ++i ) {
                        slot& s1 = r->at(i);
                        if( !s1.empty() ) {
                            for( size_t j = root->start(tbb::internal::tbb_hash<key_type>()(s1.key)); ; j=(j+1)&mask ) {
                                slot& s2 = root->at(j);
                                if( s2.empty() ) {
        ...
        void* ets_base<ETS_key_type>::table_lookup( bool& exists ) {
            ...
            void* found;
            size_t h = tbb::internal::tbb_hash<key_type>()(k);
    ...
    template<typename SegmentedContainer, typename Value >
        ...
        */
        template<typename U>
        struct ets_element {
            tbb::internal::aligned_space<U> my_space;
            bool is_built;
