...
public:
    ...
    opencl_async_msg() : my_callback_flag_ptr( std::make_shared< tbb::internal::atomic<bool>>() ) {
        my_callback_flag_ptr->store<tbb::internal::relaxed>(false);
    }

    explicit opencl_async_msg( const T& data ) : my_data(data), my_callback_flag_ptr( std::make_shared<tbb::internal::atomic<bool>>() ) {
        my_callback_flag_ptr->store<tbb::internal::relaxed>(false);
    }

    opencl_async_msg( const T& data, cl_event event ) : my_data(data), my_event(event), my_is_event(true), my_callback_flag_ptr( std::make_shared<tbb::internal::atomic<bool>>() ) {
        my_callback_flag_ptr->store<tbb::internal::relaxed>(false);
...
private:
    ...
    std::shared_ptr< tbb::internal::atomic<bool> > my_callback_flag_ptr;
};
...
public:
    ...
    opencl_async_msg<void*, Factory> receive(const cl_event *e) {
        ...
        if (my_curr_device_id.load<tbb::internal::relaxed>() != opencl_device::host) {
            map_memory(*my_factory->devices().begin(), d);
            my_curr_device_id.store<tbb::internal::relaxed>(opencl_device::host);
    ...
    opencl_async_msg<void*, Factory> send(opencl_device device, const cl_event *e) {
        ...
        if (!my_factory->is_same_context(my_curr_device_id.load<tbb::internal::acquire>(), device_id)) {
            {
                tbb::spin_mutex::scoped_lock lock(my_sending_lock);
                if (!my_factory->is_same_context(my_curr_device_id.load<tbb::internal::relaxed>(), device_id)) {
                    ...
                    my_host_ptr = NULL;
                    my_curr_device_id.store<tbb::internal::release>(device_id);
                }
            }
...
protected:
    cl_mem my_cl_mem;
    tbb::internal::atomic<opencl_device::device_id_type> my_curr_device_id;
    void* my_host_ptr;
    Factory *my_factory;
