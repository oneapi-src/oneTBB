...
protected:
    ...
    class node : public node_base {
        tbb::internal::aligned_space<value_type> my_value;
    public:
...
inline bool operator==(const tbb::concurrent_hash_map<Key, T, HashCompare, A1> &a, const tbb::concurrent_hash_map<Key, T, HashCompare, A2> &b) {
    if(a.size() != b.size()) return false;
    typename tbb::concurrent_hash_map<Key, T, HashCompare, A1>::const_iterator i(a.begin()), i_end(a.end());
    typename tbb::concurrent_hash_map<Key, T, HashCompare, A2>::const_iterator j, j_end(b.end());
    for(; i != i_end; ++i) {
