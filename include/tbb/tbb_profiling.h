...
namespace tbb {
    namespace internal {
        ...
        // two template arguments are to workaround /Wp64 warning with tbb::internal::atomic specialized for unsigned type
        template <typename T, typename U>
        inline void itt_store_word_with_release(tbb::internal::atomic<T>& dst, U src) {
        ...
        template <typename T>
        inline T itt_load_word_with_acquire(const tbb::internal::atomic<T>& src) {
#if TBB_USE_THREADING_TOOLS
            // This assertion should be replaced with static_assert
            __TBB_ASSERT(sizeof(T) == sizeof(void *), "Type must be word-sized.");
