...
public:
    ...
    async_storage() : my_graph(nullptr) {
        my_data_ready.store<tbb::internal::relaxed>(false);
    }
    ...
    async_storage(C&& data) : my_graph(nullptr), my_data( std::forward<C>(data) ) {
        ...
        my_data_ready.store<tbb::internal::relaxed>(true);
    }
    ...
    bool set(C&& data) {
...

        {
            ...
            if (my_data_ready.load<tbb::internal::relaxed>()) {
                __TBB_ASSERT(false, "double set() call");
                return false;
            }

            my_data = std::forward<C>(data);
            my_data_ready.store<tbb::internal::release>(true);
        }
    ...
    task* subscribe(async_storage_client& client, graph& g) {
        if (! my_data_ready.load<tbb::internal::acquire>())
        {
            tbb::spin_mutex::scoped_lock locker(my_mutex);
        ...
        __TBB_ASSERT(my_data_ready.load<tbb::internal::relaxed>(), "data is NOT ready");
...
private:
    ...
    tbb::internal::atomic<bool> my_data_ready;
    T my_data;
