...
public:
  measurements() { clear(); }
  void clear() { _secPerFrame.clear(); }
  void start() { _startTime = oneapi::tbb::tick_count::now(); }
  void stop() {
  ...
  double computeRelError() {
    ...
    std::transform(
        _secPerFrame.begin(), _secPerFrame.end(), diff.begin(),
        [averageTimePerFrame](double x) { return (x - averageTimePerFrame); });
    double sumOfSquareDiff =
