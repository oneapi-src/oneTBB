...
public:
    ...
    calculate_fractal( my_graph, unlimited, [&](fractal_data_t data) -> buffer_t {
        ...
        calculate_fractal.set_name("CPU: Compute Fractal (Color)");
#endif
    }
...
public:
    mono_fractal_gpu_node(graph &g, float shift) : my_graph(g), fractal_gpu_base_type(g),
    program("fractal.cl"),
...
#if defined(TBB_PREVIEW_FLOW_GRAPH_TRACE)
        fractal_compute_node.set_name("GPU: Compute Fractal (Grey)");
#endif
    }
};
...
public:
    stereo_node(graph &g) : my_graph(g), stereo_base_type(g),
    program("stereo.cl"),
...
#if defined(TBB_PREVIEW_FLOW_GRAPH_TRACE)
        left_transform.set_name("GPU: Left eye filter");
        right_transform.set_name("GPU: Right eye filter");
        image_merge.set_name("GPU: Merge stereo image");
#endif
    }
};
...
public:
    ...
    tbb::flow::opencl_device operator()(tbb::flow::opencl_factory<DeviceFilter>& f) {
        ...
        if (it == devices.cend()) {
            std::cout << "Info: could not find any GPU devices. Choosing the first available device (default behaviour)." << '\n';
...
int main() {
...
#if defined(VERBOSE)
            std::cout << "Hello from source node " << x0 << " ," << y0 << " ," << tag << '\n';
#endif
            tag++;
            return true;
        }
    ...
    stereo_mfn_t fractalInput(g, unlimited, [&]( const stereo_mfn_t::input_type &in, stereo_mfn_t::output_ports_type &ports ) {
        opencl_buffer<cl_uchar> oclImage1(tile_width*tile_height*channelsPerPixel);
        opencl_buffer<cl_uchar> oclImage2(tile_width*tile_height*channelsPerPixel);
        NDRange rangeList = { tile_width, tile_height };
        /* render left Image */
    ...
    function_node<buffer_t, buffer_t> image_display(g, tbb::flow::serial, [&](buffer_t b) {
        static int frame = 0;
        int tag = std::get<1>(b);
...
#if defined(TBB_PREVIEW_FLOW_GRAPH_TRACE)
    rj.set_name("Join input");
    rj_gpu_token.set_name("Join GPU output");
    image_display.set_name("Display");
    gpu_token_recycle.set_name("GPU token recycle");
    cpu_token_recycle.set_name("CPU token recycle");
    dispatch_node.set_name("Dispatcher");
    token_buffer.set_name("Token buffer");
    render_fractal_cpu.set_name("Fractal CPU");
    render_left_fractal.set_name("Left Fractal GPU");
    render_right_fractal.set_name("Right Fractal GPU");
    tileSource.set_name("Tile source");
    stereo.set_name("Stereo filter");
    fractalInput.set_name("GPU dispatcher");
#endif
