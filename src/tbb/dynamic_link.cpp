...
#if __USE_TBB_ATOMICS
    typedef ::tbb::internal::atomic<size_t> atomic_incrementer;
    void init_atomic_incrementer( atomic_incrementer & ) {}

    static void atomic_once( void( *func ) (void), tbb::internal::atomic< tbb::internal::do_once_state > &once_state ) {
        tbb::internal::atomic_do_once( func, once_state );
    }
    #define ATOMIC_ONCE_DECL( var ) tbb::internal::atomic< tbb::internal::do_once_state > var
#else
