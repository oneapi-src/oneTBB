...
static generic_scheduler& create_coroutine(generic_scheduler& curr) {
    ...
    generic_scheduler& s = *generic_scheduler::create_worker(*curr.my_market, curr.my_arena_index, /* genuine = */ false);
    governor::assume_scheduler(&curr);

    // Prepare scheduler.
    s.my_arena = curr.my_arena;
    s.my_dummy_task->prefix().context = s.my_arena->my_default_ctx;
    ...
    s.my_arena->my_references += arena::ref_external;
