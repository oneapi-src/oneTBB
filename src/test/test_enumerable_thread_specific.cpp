...
static Harness::atomic<int> construction_counter;
static Harness::atomic<int> destruction_counter;
...
static Harness::atomic<int> gThrowValue;
...
static Harness::atomic<int> FinitCounter;
...
void do_tbb_threads( int max_threads, flogged_ets a[] ) {
    std::vector< tbb::internal::tbb_thread * > threads;

    for (int p = 0; p < max_threads; ++p) {
        threads.push_back( new tbb::internal::tbb_thread ( set_body( a ) ) );
    }
