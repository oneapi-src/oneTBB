...
struct TestArenaEntryBody : FPModeContext {
    Harness::atomic<int> &my_stage; // each execute increases it
    std::stringstream my_id;
    bool is_caught, is_expected;
    enum { arenaFPMode = 1 };

    TestArenaEntryBody(Harness::atomic<int> &s, int idx, int i)  // init thread-specific instance
    :   FPModeContext(idx+i)
    ,   my_stage(s)
    ,   is_caught(false)
...
public:
    ForEachArenaEntryBody(tbb::task_arena &a, Harness::atomic<int> &c)
...
struct TestAttachBody : NoAssign {
    ...
    void operator()( int idx ) const {
        my_idx = idx;
        int default_threads = Harness::task_scheduler_init::default_num_threads();
        ...
        { // attach to an arena created via task_scheduler_init
            Harness::task_scheduler_init init( idx+1 );
...
//--------------------------------------------------//
#if __TBB_TASK_ISOLATION && __TBB_LEGACY_MODE
...
namespace TestIsolatedExecuteNS {
    ...
    class ParForBody : NoAssign {
        ...
        Harness::atomic<bool> &myIsStolen;
    public:
        ParForBody( bool outer_isolation, tbb::enumerable_thread_specific<int> &ets, Harness::atomic<bool> &is_stolen )
    ...
    class OuterParFor : NoAssign {
        ...
        Harness::atomic<bool> &myIsStolen;
    public:
        OuterParFor( bool outer_isolation, Harness::atomic<bool> &is_stolen ) : myOuterIsolation( outer_isolation ), myIsStolen( is_stolen ) {}
        void operator()() const {
    ...
    void HeavyMixTest() {
        Harness::task_scheduler_init init( Harness::task_scheduler_init::default_num_threads() < 3 ? 3 : Harness::task_scheduler_init::automatic );
    ...
    struct ExceptionTestBody : NoAssign {
        tbb::enumerable_thread_specific<int>& myEts;
        Harness::atomic<bool>& myIsStolen;
        ExceptionTestBody( tbb::enumerable_thread_specific<int>& ets, Harness::atomic<bool>& is_stolen )
    ...
    class TestEnqueueTask : public tbb::task {
        ...
        Harness::atomic<int>& completed;
    public:
        static const int N = 100;

        TestEnqueueTask(bool enq, tbb::enumerable_thread_specific<bool>& exe, Harness::atomic<int>& c)
    ...
    class TestEnqueueIsolateBody : NoCopy {
        ...
        Harness::atomic<int>& completed;
    public:
        TestEnqueueIsolateBody(tbb::enumerable_thread_specific<bool>& exe, Harness::atomic<int>& c)
...
void TestIsolatedExecute() {
    ...
    int num_threads = min( Harness::task_scheduler_init::default_num_threads(), 3 );
    {
        // Too many threads require too many work to reproduce the stealing from outer level.
        Harness::task_scheduler_init init( max(num_threads, 7) );
    ...
    Harness::task_scheduler_init init(num_threads);
    REMARK("."); TestIsolatedExecuteNS::HeavyMixTest();
    REMARK("."); TestIsolatedExecuteNS::ContinuationTest();
...
public:
    TestMultipleWaitsArenaWait( int idx, int bunch_size, int num_tasks, tbb::task** waiters, Harness::atomic<int>& processed )
...
private:
    ...
    int my_num_tasks;
    tbb::task** my_waiters;
    Harness::atomic<int>& my_processed;
};

class TestMultipleWaitsThreadBody : NoAssign {
public:
    TestMultipleWaitsThreadBody( int bunch_size, int num_tasks, tbb::task_arena& a, tbb::task** waiters, Harness::atomic<int>& processed )
    ...
    for ( int repeats = 0; repeats<10; ++repeats ) {
        ...
        for ( int bunch = 0; bunch < num_bunches-1; ++bunch ) {
            ...
            for ( int i = 0; i<bunch_size; ++i ) {
                ...
                tbb::internal::tbb_thread( TestMultipleWaitsThreadBody( bunch_size, num_tasks, a, tasks, processed ), idx++ ).detach();
            }
        }
...
void TestMultipleWaits() {
    ...
    const int max_threads = min( 16, Harness::task_scheduler_init::default_num_threads() );
...
void TestSmallStackSize() {
    Harness::task_scheduler_init init(Harness::task_scheduler_init::automatic,
...
void TestDefaultCreatedWorkersAmount() {
    NativeParallelFor(1, NativeParallelForBody(Harness::task_scheduler_init::default_num_threads()));
}

void TestAbilityToCreateWorkers(int thread_num) {
    Harness::task_scheduler_init init_market_with_necessary_amount_plus_one(thread_num);
...
struct MyObserver: public tbb::task_scheduler_observer {
    ...
    tbb::task_arena& my_arena;
    Harness::atomic<int>& my_failure_counter;
    Harness::atomic<int>& my_counter;

    MyObserver(tbb::task_arena& a,
        tbb::enumerable_thread_specific<tbb::task_arena*>& tls,
        Harness::atomic<int>& failure_counter,
        Harness::atomic<int>& counter)
...
void TestArenaWorkersMigrationWithNumThreads(int n_threads = 0) {
    if (n_threads == 0) {
        n_threads = Harness::task_scheduler_init::default_num_threads();
    }
...
void TestArenaWorkersMigration() {
    ...
    if (Harness::task_scheduler_init::default_num_threads() != 4) {
...
{   
    ...
    for(int i = 0; i < 8; ++i)
    {
        ...
        {
            CheckArenaNumThreads& t = *new( tbb::task::allocate_root() ) CheckArenaNumThreads(p, reserved_for_masters);
            tbb::internal::tbb_thread thr(EnqueueTaskIntoTaskArena(t, a));
            CheckArenaNumThreads::m_barrier.wait();
            a.debug_wait_until_empty();
            thr.join();
        }
...
int TestMain() {
    ...
    Harness::task_scheduler_init init_market_p_plus_one(MaxThread + 1);
