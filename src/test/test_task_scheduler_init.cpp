...
struct TestNoWorkerSurplusRun {
    void operator() (int) const {
        const unsigned THREADS = tbb::internal::tbb_thread::hardware_concurrency()*2/3;
        for (int j=0; j<10; j++) {
...
namespace TestBlockingTerminateNS {
    ...
    static Harness::atomic<int> gSeed;
    static Harness::atomic<int> gNumSuccesses;
