...
public:
    AsyncActivity(int num_) : m_numAsyncThreads(num_) {
        for (int i = 0; i < m_numAsyncThreads ; ++i) {
            m_asyncThreads.push_back( new tbb::internal::tbb_thread(AsyncActivity::asyncLoop, this) );
        }
    }
    ~AsyncActivity() {
        for (int i = 0; i < m_numAsyncThreads; ++i) {
...
private:
    ...
    std::vector<tbb::internal::tbb_thread*> m_asyncThreads;
};
...
public:
    EpochAsyncActivity(int num_, Harness::atomic<int>& e_) : m_numAsyncThreads(num_), m_globalEpoch(e_) {
        for (int i = 0; i < m_numAsyncThreads ; ++i) {
            m_asyncThreads.push_back( new tbb::internal::tbb_thread(EpochAsyncActivity::asyncLoop, this) );
        }
    }
    ~EpochAsyncActivity() {
        for (int i = 0; i < m_numAsyncThreads; ++i) {
...
private:
    ...
    const int m_numAsyncThreads;
    Harness::atomic<int>& m_globalEpoch;
    tbb::concurrent_bounded_queue<void*> m_ctxQueue;
    std::vector<tbb::internal::tbb_thread*> m_asyncThreads;
};

struct EpochSuspendBody {
    EpochSuspendBody(EpochAsyncActivity& a_, Harness::atomic<int>& e_, int& le_) :
...
void TestSuspendResume() {
    Harness::atomic<int> global_epoch; global_epoch = 0;
    ...
    Harness::atomic<int> inner_par_iters, outer_par_iters;
...
void TestCleanupMaster() {
    ...
    Harness::atomic<int> iter_spawned;
    Harness::atomic<int> iter_executed;
    ...
    for (int i = 0; i < 100; i++) {
        ...
        NativeParallelFor(N, [&asyncActivity, &tg, &iter_spawned, &iter_executed](int j) {
            Harness::task_scheduler_init init(Harness::task_scheduler_init::deferred);
            if (Harness::task_scheduler_init::default_num_threads() == 1) {
                init.initialize(2);
            }
...
void TestNativeThread() {
    ...
    int num_threads = Harness::task_scheduler_init::default_num_threads();
    tbb::task_arena arena(num_threads);
    tbb::task_group tg;
    Harness::atomic<int> iter = 0;
...
int TestMain() {
    ...
    Harness::task_scheduler_init init(max(Harness::task_scheduler_init::default_num_threads(), 16));
