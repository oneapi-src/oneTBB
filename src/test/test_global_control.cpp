...
void RunWorkersLimited(int tsi_max_threads, size_t parallelism, bool wait)
{
    ...
    const size_t expected_threads = tsi_max_threads>0?
        ...
        : ( tbb::internal::tbb_thread::hardware_concurrency()==1? 1 : parallelism );
    Harness::ExactConcurrencyLevel::check(expected_threads);
}

class blocking_task_scheduler_init {
    Harness::task_scheduler_init init;
public:
    blocking_task_scheduler_init(int num_threads = Harness::task_scheduler_init::automatic) : init(num_threads) {}
...
void TestWorkers(size_t curr_par)
{
    ...
    ASSERT(max(2U, tbb::internal::tbb_thread::hardware_concurrency()) == max_parallelism, NULL);
    {
        const unsigned h_c = tbb::internal::tbb_thread::hardware_concurrency();
        ...
        if (h_c > 1)
            TSI_and_RunWorkers(Harness::task_scheduler_init::automatic, min(h_c, curr_par), curr_par);
...
void TestWorkersConstraints() {
    ...
    for (int wait=0; wait<2; wait++) {
        for (size_t num=2; num<limit_par; num++)
            RunWorkersLimited(Harness::task_scheduler_init::automatic, num, wait==1);
...
void TestAutoInit()
{
    const size_t max_parallelism =
        tbb::global_control::active_value(tbb::global_control::max_allowed_parallelism);
    const unsigned expected_threads = tbb::internal::tbb_thread::hardware_concurrency()==1?
...
public:
    FFTask(Harness::atomic<int> *counter_) : counter(counter_) {}
};

class WaiterTask: public tbb::task {
    Harness::atomic<bool> *flag;
    tbb::task* execute() __TBB_override {
        while(!*flag)
            __TBB_Yield();
       return NULL;
...
public:
    WaiterTask(Harness::atomic<bool> *flag_) : flag(flag_) {}
};

class WorkAndEnqueueTask: public tbb::task {
    Harness::atomic<int> *counter;
    Harness::atomic<bool> *signalToLeave;
    tbb::task* execute() __TBB_override {
        RunParallelWork();
        *signalToLeave = true;
...
public:
    ...
    WorkAndEnqueueTask(Harness::atomic<int> *counter_, Harness::atomic<bool> *signal_)
...
public:
    ...
    Harness::SpinBarrier *barr;
    Harness::atomic<int> *cnt;

    FFTasksRun(Harness::SpinBarrier *b, Harness::atomic<int> *c) :
        barr(b), cnt(c) {}
    void operator()(int id) const {
        if (id)
            enqTasks(id);
...
{
    {
        ...
        Harness::atomic<int> flag;
        Harness::atomic<bool> taskDoneFlag;
        flag = 0;
        taskDoneFlag = false;
...
public:
    CountWorkersTask(Harness::atomic<bool> *flag_) : flag(flag_) {}
...
Harness::atomic<int> activeArenas;
...
struct ArenaRun: NoAssign {
    ...
    ArenaRun(Harness::atomic<int> *counter_) : counter(counter_) {}
    void operator()() const {
        (*counter)++;
    }
};
...
struct ArenaUserRun: NoAssign {
    ...
    ArenaUserRun(tbb::task_arena *a, Harness::SpinBarrier *b, Harness::atomic<int> *c) :
...
{
    Harness::task_scheduler_init tsi(2);
    Harness::atomic<int> counter;
    Harness::atomic<bool> waitFlag;
...
int TestMain()
{
    ...
    bool excessHC;
    {
        Harness::task_scheduler_init t(h_c+1);
