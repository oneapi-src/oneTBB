...
#define HARNESS_DEFAULT_MIN_THREADS (Harness::task_scheduler_init::default_num_threads())
#define HARNESS_DEFAULT_MAX_THREADS (4*Harness::task_scheduler_init::default_num_threads())
#if __bg__
...
public:
    ...
    void operator()(const int /*threadID*/) const {
        Harness::task_scheduler_init sch(MaxThread);
        CallParallelFor();
        if (wait_workers) {
...
public:
    ...
    void operator()(const int /*threadID*/) const {
        ...
        Harness::task_scheduler_init tsi(create_tsi? 2 : Harness::task_scheduler_init::deferred);
        CallParallelFor();
        if (blocking) {
...
{
    Harness::task_scheduler_init sch(2);
    CallParallelFor();
    for (int i=0; i<2; i++)
...
#endif
        for (int i=0; i<100; i++) {
            Harness::task_scheduler_init sch(1);
            for (int k=0; k<10; k++) {
...
void TestNestingTSI()
{
    ...
    for (int i=0; i<2; i++) {
        Harness::task_scheduler_init schBlock(2);
        CallParallelFor();
        Harness::task_scheduler_init schBlock1(2);
        CallParallelFor();
        if (i)
            schBlock1.terminate();
        else {
...
int TestMain()
{
    ...
    for (int threads=MinThread; threads<=MaxThread; threads+=MinThread) {
        for (int i=0; i<20; i++) {
            ...
            {
                Harness::task_scheduler_init sch(threads);
                bool ok = sch.blocking_terminate(std::nothrow);
                ASSERT(ok, NULL);
            }
            Harness::task_scheduler_init sch(threads);
