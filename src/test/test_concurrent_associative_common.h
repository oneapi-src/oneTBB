...
public:
    TestRange( const std::list<ValueType> &lst, std::vector< Harness::atomic<bool> > &marks ) : my_lst( lst ), my_marks( marks ) {
...
void TestSetCommonTypes() {
    ...
    std::list< Harness::atomic<int> > arrTbb(NUMBER);
    int seq = 0;
    for ( std::list< Harness::atomic<int> >::iterator it = arrTbb.begin(); it != arrTbb.end(); ++it, ++seq ) *it = seq;
...
void TestMapCommonTypes() {
    ...
    std::list< std::pair< const int, Harness::atomic<int> > > arrIntTbb;
