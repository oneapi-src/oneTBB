...
Harness::atomic<long> MyDataCount;
...
Harness::atomic<YourTable::size_type> InsertEraseCount[IE_SIZE];
...
typedef Harness::atomic<unsigned char> AtomicByte;
...
template<typename base_alloc_t, typename count_t = Harness::atomic<size_t> >
...
class test_range : NoAssign {
    ...
    std::vector< Harness::atomic<bool> >& my_marks;
public:
    test_range( Table &c, const std::list<value_type> &lst, std::vector< Harness::atomic<bool> > &marks ) : my_c(c), my_lst(lst), my_marks(marks) {
...
void Examine( Table c, const std::list<typename Table::value_type> &lst) {
    ...
    std::vector< Harness::atomic<bool> > marks( lst.size() );
...
void TestCPP11Types() {
    ...
    typedef std::pair< const int, Harness::atomic<int> > int_tbb_t;
    std::list<int_tbb_t> arrIntTbb;
    for ( int i=0; i<NUMBER; ++i ) {
        Harness::atomic<int> b;
        b = NUMBER-i;
...
struct hash_map_move_traits : default_container_traits {
    ...
    struct hash_compare {
        ...
        size_t hash( const T& k ) const {
            using tbb::interface5::tbb_hasher;
            return tbb_hasher(k);
        }
    };
