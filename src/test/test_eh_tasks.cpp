...
int TestMain () {
    ...
    MinThread = min(NUM_ROOTS_IN_GROUP, min(Harness::task_scheduler_init::default_num_threads(), max(2, MinThread)));
    MaxThread = min(NUM_ROOTS_IN_GROUP, max(MinThread, min(Harness::task_scheduler_init::default_num_threads(), MaxThread)));
