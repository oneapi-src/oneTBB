...
private:
    ...
    Harness::atomic<ScopedLock*> q_tail;
...
void QueuingMutex::ScopedLock::Acquire( QueuingMutex& m, size_t test_mode )
{
    ...
    ScopedLock* pred = m.q_tail.fetch_and_store<Harness::release>(this);
    if( pred ) {
#if TBB_USE_ASSERT
...
{
    if( !next ) {
        if( this == mutex->q_tail.compare_and_swap<Harness::release>(NULL, this) ) {
...
Harness::atomic<size_t> n_sleepers;
...
void TestDestructor() {
#if TBB_USE_EXCEPTIONS
    Harness::task_scheduler_init init(NTHRS_USED_IN_DESTRUCTOR_TEST);
