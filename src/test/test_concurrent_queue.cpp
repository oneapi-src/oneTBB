...
static Harness::atomic<long> FooConstructed;
static Harness::atomic<long> FooDestroyed;
...
static Harness::atomic<long> FooExConstructed;
static Harness::atomic<long> FooExDestroyed;
static Harness::atomic<long> serial_source;
...
/** [0] = pop_if_present that failed
    ...
    [2] = pop */
static Harness::atomic<long> PopKind[3];
...
#if TBB_USE_EXCEPTIONS
Harness::atomic<size_t> num_pushed;
Harness::atomic<size_t> num_popped;
Harness::atomic<size_t> failed_pushes;
Harness::atomic<size_t> failed_pops;
...
void TestTypes() {
    ...
    std::vector< Harness::atomic<int> > arrTbb;
    for (int i = 0; i < NUMBER; ++i) {
        Harness::atomic<int> a;
        a = i;
        arrTbb.push_back(a);
    }
    TestTypes(arrInt);
