...
#include "tbb/task.h"
#include "harness_task_scheduler_init.h"

#if __TBB_LEGACY_MODE
#include "tbb/compat/condition_variable"
#include "tbb/mutex.h"
#elif !TBB_IMPLEMENT_CPP0X
...
struct harness_graph_executor {
    ...
    static Harness::atomic<size_t> execute_count;
    static Harness::atomic<size_t> current_executors;
    ...
    template< typename RW >
    struct tfunctor {
        Harness::atomic<size_t> my_execute_count;
...
template< typename InputType, typename OutputType >
Harness::atomic<size_t> harness_graph_executor<InputType, OutputType>::execute_count;
...
template< typename InputType, typename OutputType >
Harness::atomic<size_t> harness_graph_executor<InputType, OutputType>::current_executors;
...
template< typename InputType, typename OutputTuple >
Harness::atomic<size_t> harness_graph_multifunction_executor<InputType, OutputTuple>::execute_count;
...
template< typename InputType, typename OutputTuple >
Harness::atomic<size_t> harness_graph_multifunction_executor<InputType, OutputTuple>::current_executors;
...
template< typename T >
struct harness_counting_receiver : public tbb::flow::interface11::receiver<T>, NoAssign {

    Harness::atomic< size_t > my_count;
    T max_value;
    size_t num_copies;
    ...
    typedef typename tbb::flow::interface11::receiver<T>::built_predecessors_type built_predecessors_type;
    built_predecessors_type mbp;
    built_predecessors_type &built_predecessors() __TBB_override { return mbp; }
    typedef typename tbb::flow::interface11::receiver<T>::predecessor_list_type predecessor_list_type;
    typedef typename tbb::flow::interface11::receiver<T>::predecessor_type predecessor_type;
...
//! Counts the number of puts received
template< typename T >
struct harness_mapped_receiver : public tbb::flow::interface11::receiver<T>, NoCopy {

    Harness::atomic< size_t > my_count;
    T max_value;
    size_t num_copies;
    typedef tbb::concurrent_unordered_map< T, Harness::atomic< size_t > > map_type;
    map_type *my_map;
    tbb::flow::graph& my_graph;
...
//! Counts the number of puts received
template< typename T >
struct harness_counting_sender : public tbb::flow::interface11::sender<T>, NoCopy {

    typedef typename tbb::flow::interface11::sender<T>::successor_type successor_type;
    Harness::atomic< successor_type * > my_receiver;
    Harness::atomic< size_t > my_count;
    Harness::atomic< size_t > my_received;
    size_t my_limit;
...
#if TBB_DEPRECATED_FLOW_NODE_EXTRACTION
    typedef typename tbb::flow::interface11::sender<T>::successor_list_type successor_list_type;
    typedef typename tbb::flow::interface11::sender<T>::built_successors_type built_successors_type;
...
Harness::atomic<int> serial_fn_state0;
Harness::atomic<int> serial_fn_state1;
Harness::atomic<int> serial_continue_state0;

template<typename T>
struct serial_fn_body {
    Harness::atomic<int> *_flag;
    serial_fn_body(Harness::atomic<int> &myatomic) : _flag(&myatomic) { }
...
struct serial_continue_body {
    Harness::atomic<int> *_flag;
    serial_continue_body(Harness::atomic<int> &myatomic) : _flag(&myatomic) {}
...
void test_resets() {
    const int NN = 3;
    Harness::task_scheduler_init init(4);
...
protected:
    ...
    typedef Harness::atomic<size_t> count_t;
...
public:
    ...
    void operator()(int) const {
        tbb::internal::tbb_thread::id this_id = tbb::internal::thread_get_id_v3();
...
public:
    Harness::atomic<unsigned> my_body_count;
    ...
    void operator()(const tbb::internal::tbb_thread::id& input, gateway_type&) {
        increase_and_check(input);
    }

    output_tuple_type operator()(const tbb::internal::tbb_thread::id& input) {
        increase_and_check(input);
        return output_tuple_type();
    }

private:
    void increase_and_check(const tbb::internal::tbb_thread::id& input) {
        ++my_body_count;
        tbb::internal::tbb_thread::id body_thread_id = tbb::internal::thread_get_id_v3();
...
typedef tbb::mutex lightweight_mutex;
#elif !TBB_IMPLEMENT_CPP0X
using std::condition_variable;
using std::unique_lock;
typedef std::mutex lightweight_mutex;
#endif

// If no chance to have condition variable on the system, then skip the test
#if __TBB_LEGACY_MODE || !TBB_IMPLEMENT_CPP0X
...
public:
    Harness::atomic<unsigned> my_body_count;
    Harness::atomic<unsigned> my_lightweight_count;
    Harness::atomic<unsigned> my_task_count;
...
private:
    void increase_and_check(const tbb::internal::tbb_thread::id& /*input*/) {
        ...
        if(is_task) {
            ...
            unique_lock<lightweight_mutex> lock(m);
...
#endif // __TBB_LEGACY_MODE || (!__TBB_LEGACY_MODE && !TBB_IMPLEMENT_CPP0X)
...
void test_lightweight(unsigned N) {
    ...
    test_limited_lightweight_execution<NodeType>(N, (std::min)(tbb::internal::tbb_thread::hardware_concurrency() / 2, N/2));
#endif
}

template<template<typename, typename, typename, typename> class NodeType>
void test(unsigned N) {
    typedef tbb::internal::tbb_thread::id input_type;
