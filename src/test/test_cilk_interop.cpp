...
int Fib ( int n ) {
    if( n < 2 ) {
        if ( g_sim ) {
            Harness::task_scheduler_init tsi(P_nested);
        }
        return n;
    } else {
        Harness::task_scheduler_init *tsi = NULL;
        tbb::task *cur = NULL;
        if ( g_sim ) {
            if ( n % 2 == 0 ) {
                if ( g_sim == tbbsched_auto_only || (g_sim == tbbsched_mixed && n % 4 == 0) ) {
                    ...
                    tsi = new Harness::task_scheduler_init(P_nested);
                }
            }
        }
        int x, y;
...
void RunSandwich( bool sandwich ) {
    g_sandwich = sandwich;
    Harness::task_scheduler_init init(P_outer);
    int m = TBB_Fib(N);
