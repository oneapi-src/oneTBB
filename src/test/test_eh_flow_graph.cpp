...
Harness::atomic<unsigned> nExceptions;
Harness::atomic<intptr_t> g_TGCCancelled;
...
class test_source_body : WaitThrow<serial_type, TType> {
    ...
    Harness::atomic<int> *my_current_val;
    int my_mult;
public:
    test_source_body(Harness::atomic<int> &my_cnt, int multiplier = 1) : my_current_val(&my_cnt), my_mult(multiplier) {
...
class test_source_body<tbb::flow::continue_msg, TType> : WaitThrow<serial_type, TType> {
    ...
    Harness::atomic<int> *my_current_val;
public:
    test_source_body(Harness::atomic<int> &my_cnt) : my_current_val(&my_cnt) { }
...
public:
    absorber_body(Harness::atomic<int> &my_cnt) : my_count(&my_cnt) { }
...
class multifunction_node_body : WaitThrow<Conc,T> {
    ...
    Harness::atomic<int> *my_count;
public:
    multifunction_node_body(Harness::atomic<int> &my_cnt) : my_count(&my_cnt) { }
...
class tuple_test_source_body : WaitThrow<serial_type, TType> {
    ...
    Harness::atomic<int> *my_current_val;
public:
    tuple_test_source_body(Harness::atomic<int> &my_cnt) : my_current_val(&my_cnt) { }
...
void run_one_source_node_test(bool throwException, bool flog) {
    ...
    Harness::atomic<int> source_body_count;
    Harness::atomic<int> absorber_body_count;
    source_body_count = 0;
...
struct AttachPoint<NodeType,ItemType,indx,multifunc_node_type> {
    static tbb::flow::interface11::sender<ItemType> &GetSender(NodeType &n) {
...
run_one_functype_node_test(bool throwException, bool flog, const char * /*name*/) {
    ...
    Harness::atomic<int> source0_count;
    Harness::atomic<int> source1_count;
    Harness::atomic<int> sink0_count;
    Harness::atomic<int> sink1_count;
    Harness::atomic<int> test_count;
...
void run_one_continue_node_test (bool throwException, bool flog) {
    ...
    Harness::atomic<int> source_count;
    Harness::atomic<int> test_count;
    Harness::atomic<int> sink_count;
