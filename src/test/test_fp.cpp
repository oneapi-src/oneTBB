...
void CheckNoSseStatusPropagation() {
    ...
    ctl.set_env();
    const int num_threads = Harness::task_scheduler_init::default_num_threads();
...
void TestFpuEnvPropagation () {
    const int p = Harness::task_scheduler_init::default_num_threads();
...
struct TestGlobalIsolatedContextNativeLoopBody {
    void operator()( int threadId ) const {
        ...
        static tbb::spin_mutex rootAllocMutex;
        rootAllocMutex.lock();
