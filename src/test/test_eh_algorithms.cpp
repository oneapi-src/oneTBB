...
#include "harness_barrier.h" // Harness::barrier
...
Harness::atomic<intptr_t> g_FedTasksCount; // number of tasks added by parallel_do feeder
Harness::atomic<intptr_t> g_OuterParCalls;  // number of actual invocations of the outer construct executed.
Harness::atomic<intptr_t> g_TGCCancelled;  // Number of times a task sees its group cancelled at start
...
struct StatsCounters {
    Harness::atomic<size_t> my_total_created;
    Harness::atomic<size_t> my_total_deleted;
    StatsCounters() {
        my_total_created = 0;
        my_total_deleted = 0;
    }
...
void RunParForAndReduceTests () {
    ...
    Harness::task_scheduler_init init (g_NumThreads);
...
Harness::atomic<size_t> g_AllocatedCount; // Number of currently allocated buffers
Harness::atomic<size_t> g_TotalCount; // Total number of allocated buffers
...
#if TBB_USE_EXCEPTIONS && __TBB_LEGACY_MODE
...
int TestMain () {
    ...
    MinThread = min(Harness::task_scheduler_init::default_num_threads(), max(2, MinThread));
    MaxThread = max(MinThread, min(Harness::task_scheduler_init::default_num_threads(), MaxThread));
