...
// Serial parallel_for is available only in legacy mode
#define TBB_PREVIEW_SERIAL_SUBSET __TBB_LEGACY_MODE
...
static Harness::atomic<int> FooBodyCount;
...
class FooBody {
    ...
    Harness::atomic<int>* array;
    int state;
    friend class FooRange<Pad>;
    template<typename Flavor_, size_t Pad_> friend void Flog( int nthread );
    FooBody( Harness::atomic<int>* array_ ) : array(array_), state(LIVE) {}
public:
    ~FooBody() {
        --FooBodyCount;
...
static Harness::atomic<int> Array[N];
...
void test() {
    static const int thread_num = Harness::task_scheduler_init::default_num_threads();
...
template <typename Partitioner>
void test_body() {
    unsigned hw_concurrency = tbb::internal::tbb_thread::hardware_concurrency();
