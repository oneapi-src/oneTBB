...
#define _TRACE_(msg) {                                                  \
    if (Verbose) {                                                      \
        ...
        os << "[TID=" << tbb::internal::thread_get_id_v3() << "] " << msg; \
        REMARK("%s\n", os.str().c_str());                               \
    }                                                                   \
}
...
private: // DATA
    ...
    int                         myTimeoutMS;
    tbb::internal::tbb_thread             myThread;
...
struct F2_body : tbb::internal::no_assign
{
    ...
    F2_body(const F2_body& b) : tbb::internal::no_assign(), myI(b.myI), myAlive(true) {}
...
struct F3_body : tbb::internal::no_assign
{
    ...
    F3_body(const F3_body& b) : tbb::internal::no_assign(), myI(b.myI), myAlive(true) {}
...
//
    ...
    __TBB_STATIC_ASSERT( (bConvertible == CheckerTryPut<tbb::flow::interface11::receiver<T1>, T2>::value), "invalid availability of receiver<T1>->try_put(T2)" );
};
