...
#if __TBB_TEST_SKIP_PIC_MODE || (__TBB_TEST_SKIP_GCC_BUILTINS_MODE && __TBB_TEST_SKIP_ICC_BUILTINS_MODE) || !__TBB_LEGACY_MODE
#include "harness.h"
int TestMain() {
...
struct TestStruct {
    ...
    static Harness::atomic<T> gCounter;
};

// A global variable of type Harness::atomic<>
template<typename T, LoadStoreExpression E> Harness::atomic<T> TestStruct<T, E>::gCounter;
...
namespace initialization_tests {
    ...
    struct test_initialization_fixture{
        typedef Harness::atomic<T> atomic_t;
        ...
        void operator()(){
            ...
            //TODO: add use of KNOWN_ISSUE macro on SunCC 5.11
                ...
                ASSERT(char(0)==*(reinterpret_cast<char*>(this->non_zeroed_storage.begin()))
                        ,("value initialization for Harness::atomic should do zero initialization; "
        ...
        struct TestDirectInitialization : test_initialization_fixture<T> {
            void operator()(T i){
                ...
                ASSERT(i == this->non_zeroed_storage.begin()->load()
                        ,("Harness::atomic initialization failed; "
...
void TestConstExprInitializationIsTranslationTime(){
    ...
    typedef Harness::atomic<int> atomic_t;
    constexpr atomic_t a(8);
    ASSERT(a == 8,ct_init_failed_msg);

#if !__TBB_CONSTEXPR_MEMBER_FUNCTION_BROKEN
    constexpr Harness::atomic<test_constexpr_initialization_helper::white_box_ad_hoc_type> ct_atomic(10);
...
namespace TestConstExprInitializationOfGlobalObjectsHelper{
    ...
    struct tester<T> {                                                   \
        ...
        static Harness::atomic<T> static_atomic;                             \
                                                                         \
    ...
    Harness::atomic<T> tester<T>::static_atomic(ct_value);                   \
                                                                         \
...
#   if __TBB_STATIC_CONSTEXPR_INIT_BROKEN
        REPORT("Known issue: Compile-time initialization fails for static Harness::atomic variables\n");
#   else
...
void TestAlignment( const char* name ) {
    ...
    bool is_struct_size_correct = (sizeof(AlignmentChecker<T>)==2*sizeof(Harness::atomic<T>));
    ...
    if (!is_struct_size_correct){
        std::string msg = "Compiler failed to properly add padding to structure with atomic member variable?; Structure size:"+to_string(sizeof(AlignmentChecker<T>))
                + " atomic size:"+to_string(sizeof(Harness::atomic<T>)) + " type: " + to_string(name) +"\n";
        if (known_issue_condition) {
            REPORT(("Known issue: "+ msg).c_str());
        }else{
            ASSERT(false,msg.c_str());
...
void TestAtomicEnum() {
    ...
    __TBB_STATIC_ASSERT( HasImplicitConversionToInt< Harness::atomic<Color> >::value, "The implicit conversion is expected." );
}
...
#endif
    __TBB_STATIC_ASSERT( !HasImplicitConversionToInt< Harness::atomic<ScopedColor1> >::value, "The implicit conversion is not expected." );
    __TBB_STATIC_ASSERT( !HasImplicitConversionToInt< Harness::atomic<ScopedColor1> >::value, "The implicit conversion is not expected." );
    __TBB_STATIC_ASSERT( sizeof(Harness::atomic<ScopedColor1>) == sizeof(ScopedColor1), "Harness::atomic instantiated with scoped enum should have the same size as scoped enum." );
    __TBB_STATIC_ASSERT( sizeof(Harness::atomic<ScopedColor2>) == sizeof(ScopedColor2), "Harness::atomic instantiated with scoped enum should have the same size as scoped enum." );
}
...
class TestRelaxedLoadStoreAtomicBody {
    static Harness::atomic<T> s_turn,
                          s_ready;
...
template <typename T> Harness::atomic<T> TestRelaxedLoadStoreAtomicBody<T>::s_turn;
template <typename T> Harness::atomic<T> TestRelaxedLoadStoreAtomicBody<T>::s_ready;
...
public:
    Harness::atomic<T>& construct_atomic(){
        std::memset(&raw_space[0],0, sizeof(raw_space));
        uintptr_t delta = aligned ? 0 : sizeof(T)/2;
        size_t index=sizeof(T)-1;
        Harness::atomic<T>* y = reinterpret_cast<Harness::atomic<T>*>((reinterpret_cast<uintptr_t>(&raw_space[index+delta])&~index) - delta);
...
struct LoadStoreTraits<T, UseOperators> {
    static void load ( T& dst, const Harness::atomic<T>& src ) { dst = src; }
    static void store ( Harness::atomic<T>& dst, const T& src ) { dst = src; }
};

template <typename T>
struct LoadStoreTraits<T, UseImplicitAcqRel> {
    static void load ( T& dst, const Harness::atomic<T>& src ) { dst = src.load(); }
    static void store ( Harness::atomic<T>& dst, const T& src ) { dst.store(src); }
};

template <typename T>
struct LoadStoreTraits<T, UseExplicitFullyFenced> {
    static void load ( T& dst, const Harness::atomic<T>& src ) { dst = src.template load<tbb::full_fence>(); }
    static void store ( Harness::atomic<T>& dst, const T& src ) { dst.template store<tbb::full_fence>(src); }
};

template <typename T>
struct LoadStoreTraits<T, UseExplicitAcqRel> {
    static void load ( T& dst, const Harness::atomic<T>& src ) { dst = src.template load<tbb::acquire>(); }
    static void store ( Harness::atomic<T>& dst, const T& src ) { dst.template store<tbb::release>(src); }
};

template <typename T>
struct LoadStoreTraits<T, UseExplicitRelaxed> {
    static void load ( T& dst, const Harness::atomic<T>& src ) { dst = src.template load<tbb::relaxed>(); }
    static void store ( Harness::atomic<T>& dst, const T& src ) { dst.template store<tbb::relaxed>(src); }
};

template <typename T>
struct LoadStoreTraits<T, UseGlobalHelperFullyFenced> {
    static void load ( T& dst, const Harness::atomic<T>& src ) { dst = tbb::load<tbb::full_fence>(src); }
    static void store ( Harness::atomic<T>& dst, const T& src ) { tbb::store<tbb::full_fence>(dst, src); }
};

template <typename T>
struct LoadStoreTraits<T, UseGlobalHelperAcqRel> {
    static void load ( T& dst, const Harness::atomic<T>& src ) { dst = tbb::load<tbb::acquire>(src); }
    static void store ( Harness::atomic<T>& dst, const T& src ) { tbb::store<tbb::release>(dst, src); }
};

template <typename T>
struct LoadStoreTraits<T, UseGlobalHelperRelaxed> {
    static void load ( T& dst, const Harness::atomic<T>& src ) { dst = tbb::load<tbb::relaxed>(src); }
    static void store ( Harness::atomic<T>& dst, const T& src ) { tbb::store<tbb::relaxed>(dst, src); }
};
...
// Intended to check the assignment operator of Harness::atomic.
...
void TestAssignment( const char* name ) {
    TestAssignmentSignature( &Harness::atomic<T>::operator= );
...
class DekkerArbitrationBody : NoAssign, Harness::NoAfterlife {
    ...
    mutable Harness::atomic<T>* s_ready[2];
    Harness::atomic<T>& s_turn;
