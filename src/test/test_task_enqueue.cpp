...
public:
    ...
    static Harness::atomic<int> nCompletedPairs;
    static Harness::atomic<int> nOrderedPairs;
};

Harness::atomic<int> EnqueuedTask::nCompletedPairs;
Harness::atomic<int> EnqueuedTask::nOrderedPairs;
...
void TimedYield( double pause_time ) {
    ...
    while( (tbb::tick_count::now()-start).seconds() < pause_time )
        tbb::internal::thread_sleep_v3(tbb::tick_count::interval_t(pause_time));
}
...
void TestEnqueue( int p ) {
    ...
    for(int mode=0;mode<3;++mode) {
        ...
        ProgressMonitor pm;
        tbb::internal::tbb_thread thr( pm );
        if(mode==1) {
...
public:
    void operator() ( int id ) const {
        Harness::task_scheduler_init init(Harness::task_scheduler_init::default_num_threads() + 1);
...
void TestCascadedEnqueue () {
    ...
    int minNumThreads = min(Harness::task_scheduler_init::default_num_threads(), MaxNumThreads) / 2;
    int maxNumThreads = min(Harness::task_scheduler_init::default_num_threads() * 2, MaxNumThreads);
...
void TestWakeups()
{
    Harness::task_scheduler_init my(Harness::task_scheduler_init::deferred);
    if( Harness::task_scheduler_init::default_num_threads() <= NUM_TASKS )
        my.initialize(NUM_TASKS*2);
    else // workaround issue #1996 for TestCascadedEnqueue
        my.initialize(Harness::task_scheduler_init::default_num_threads()+1);
