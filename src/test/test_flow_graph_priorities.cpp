...
Harness::atomic<unsigned> g_task_num;
...
template<typename NodeType> tbb::flow::interface11::sender<int>& get_sender( NodeType& node ) { return node; }
template<> tbb::flow::interface11::sender<int>& get_sender<multi_node>( multi_node& node ) { return output_port<0>(node); }
...
struct AsyncActivity {
    ...
    tbb::internal::tbb_thread my_service_thread;
...
void test( int num_threads ) {
    ...
    if( num_threads == Harness::task_scheduler_init::default_num_threads() ) {
    ...
    Harness::task_scheduler_init init(cpu_threads);
...
struct execution_tracker_t {
    ...
    void reset() {
        prioritized_work_submitter = tbb::internal::tbb_thread::id();
        prioritized_work_started = false;
        prioritized_work_finished = false;
        prioritized_work_interrupted = false;
    }
    tbb::internal::tbb_thread::id prioritized_work_submitter;
...
void do_nested_work( const tbb::internal::tbb_thread::id& tid, const tbb::blocked_range<int>& subrange );
...
struct CommonBody {
    ...
    void operator()( const tbb::blocked_range<int>& subrange ) const {
        do_nested_work<work_type>( /*tid=*/tbb::internal::thread_get_id_v3(), subrange );
    }
    int my_body_size;
};
...
void do_node_work<PRIORITIZED_WORK>(int work_size) {
    exec_tracker.prioritized_work_submitter = tbb::internal::thread_get_id_v3();
...
template<>
void do_nested_work<PRIORITIZED_WORK>( const tbb::internal::tbb_thread::id& tid,
...
void test( int max_threads ) {
    Harness::task_scheduler_init init( max_threads );
