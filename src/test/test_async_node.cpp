...
struct place_wrapper {
    ...
    T value;
    tbb::internal::tbb_thread::id thread_id;
    tbb::task* task_ptr;

    place_wrapper( ) : value(0) {
        thread_id = tbb::internal::thread_get_id_v3();
...
Harness::atomic<int> async_body_exec_count;
Harness::atomic<int> async_activity_processed_msg_count;
Harness::atomic<int> end_body_exec_count;
...
struct counting_async_body {
    Harness::atomic<int> my_async_body_exec_count;
...
public:
    ...
    async_activity(int expected_items, bool deferred = false, int sleep_time = 50)
        ...
        is_active = !deferred;
        my_quit = false;
        tbb::internal::tbb_thread( ServiceThreadBody( this ) ).swap( my_service_thread );
    }
...
private:
    ...
    Harness::atomic< bool > is_active;

    tbb::concurrent_queue< work_type > my_work_queue;

    Harness::atomic< bool > my_quit;
...
Harness::atomic<int> main_tid_count;
...
struct spin_test {
    ...
    class end_body_type {
        typedef Output output_type;
        tbb::internal::tbb_thread::id my_main_tid;
        Harness::SpinBarrier *my_barrier;
    public:
        end_body_type(tbb::internal::tbb_thread::id t, Harness::SpinBarrier &b) : my_main_tid(t), my_barrier(&b) { }

        void operator()( const output_type & ) {
            ++end_body_exec_count;
            if (tbb::internal::thread_get_id_v3() == my_main_tid) {
               ++main_tid_count;
            }
    ...
    static int run(int nthreads, int async_expected_items = UNKNOWN_NUMBER_OF_ITEMS) {
        ...
        async_node_type offload_node( g, tbb::flow::unlimited, async_body_type( &my_async_activity ) );
#endif
        tbb::flow::function_node< output_type > end_node( g, tbb::flow::unlimited, end_body_type(tbb::internal::thread_get_id_v3(), spin_barrier) );
        tbb::flow::make_edge( start_node, offload_node );
#if __TBB_FLOW_GRAPH_CPP11_FEATURES
        tbb::flow::make_edge( offload_node, end_node );
#else
