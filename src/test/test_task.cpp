...
#include "harness_barrier.h" // Harness::atomic
...
void TestStealLimit( int nthread ) {
    ...
    Harness::task_scheduler_init init(nthread);
...
void TestAffinity( int nthread ) {
    TotalCount = 0;
    int n = Harness::task_scheduler_init::default_num_threads();
    if( n>nthread )
        n = nthread;
    Harness::task_scheduler_init init(n);
...
void TestNoteAffinityContext() {
    Harness::task_scheduler_init init(1);
    tbb::empty_task* t = new( tbb::task::allocate_root() ) tbb::empty_task;
    t->set_ref_count(2);
...
public:
    ...
    static Harness::atomic<int> execution_count;
    static Harness::atomic<int> destruction_count;
};

Harness::atomic<int> DagTask::execution_count;
Harness::atomic<int> DagTask::destruction_count;
...
void TestRelaxedOwnership( int p ) {
    ...
    if( unsigned(p)>tbb::internal::tbb_thread::hardware_concurrency() )
        return;
...
void TestUserThread( int p ) {
    ...
    for( int n_child=0; n_child<2; ++n_child ) {
        tbb::internal::tbb_thread t( RunSchedulerInstanceOnUserThread, n_child );
        t.join();
    }
}
...
void TestDispatchLoopResponsiveness() {
    ...
    // 1 - worker's nested
    // 2 - master's nested
    Harness::task_scheduler_init init(2);
    tbb::task &r = *new( tbb::task::allocate_root() ) tbb::empty_task;
...
void TestWaitDiscriminativenessWithStealing() {
    if( tbb::internal::tbb_thread::hardware_concurrency() < 2 )
        return;
...
void TestMastersIsolation ( int p ) {
    ...
    if ( p > 2 && Harness::task_scheduler_init::default_num_threads() >= p ) {
        Harness::task_scheduler_init init(p);
...
Harness::atomic<bool> g_is_critical_task_submitted;
size_t g_bypassed_critical_task_index = size_t(-1);
task* g_bypassed_task_pointer = NULL;
int g_bypassed_task_creator = -1;
Harness::atomic<bypassed_critical_task_stage_t> g_bypassed_critical_task_stage;
...
Harness::atomic<size_t> g_num_executed_from_cancelled_context;
Harness::atomic<size_t> g_num_executed_from_working_context;
