...
typedef Harness::atomic<uint_t> atomic_t;
typedef tbb::task_handle<void(*)()> handle_type;
...
public:
    ...
    void Run ( uint_t idx ) {
#if TBBTEST_USE_TBB
        Harness::task_scheduler_init init(g_MaxConcurrency);
#endif
        AssertLive();
        if ( idx == 0 ) {
...
class RunAndWaitSyncronizationTestBody : NoAssign {
    Harness::SpinBarrier& m_barrier;
    Harness::atomic<bool>& m_completed;
    task_group_type& m_tg;
public:
    RunAndWaitSyncronizationTestBody(Harness::SpinBarrier& barrier, Harness::atomic<bool>& completed, task_group_type& tg)
...
public:
    ...
    virtual void impl() const __TBB_override {
        uint_t x = ~0u;
        task_group_type tg;
        tbb::task_handle<FibTaskAsymmetricTreeWithTaskHandle>
...
public:
    ...
    virtual void impl() const __TBB_override {
        uint_t x = ~0u,
               y = ~0u;
        task_group_type tg;
        tbb::task_handle<FibTaskSymmetricTreeWithTaskHandle>
...
void TestFibWithMakeTask() {
    ...
    sum = 0;
    task_group_type tg;
    auto h1 = tbb::make_task( [&](){sum += RunFibTask<FibTaskSymmetricTreeWithTaskHandle<task_group_type> >(N);} );
    auto h2 = tbb::make_task( [&](){sum += RunFibTask<FibTaskSymmetricTreeWithTaskHandle<task_group_type> >(N);} );
    tg.run( h1 );
    tg.run_and_wait( h2 );
    ASSERT( sum == 2 * F, NULL );
}
...
void TestFibInvalidMultipleScheduling() {
    ...
    g_Sum = 0;
    task_group_type tg;
    typedef tbb::internal::aligned_space<handle_type> handle_space_t;
    ...
    for( i = 0; i < numRepeats; ++i )
        ...
        handles[i].begin()->tbb::task_handle<void(*)()>::~task_handle();
#else
...
template<> void RunFibonacciTests<tbb::structured_task_group>() {
    TestFibWithTaskHandle<tbb::structured_task_group>();
#if __TBB_CPP11_LAMBDAS_PRESENT
    TestFibWithMakeTask<tbb::structured_task_group>();
#endif
    TestFibInvalidMultipleScheduling<tbb::structured_task_group>();
}
...
void LaunchChildrenWithFunctor () {
    ...
    ASSERT( !g_Throw || exceptionCaught || status == tbb::canceled, "No exception in the child task group" );
...
void LaunchChildrenWithTaskHandle () {
    ...
    task_group_type g;
    bool exceptionCaught = false;
    typedef tbb::task_handle<ThrowingTask> throwing_handle_type;
    tbb::internal::aligned_space<throwing_handle_type,NUM_CHORES> handles;
    for( unsigned i = 0; i < NUM_CHORES; ++i ) {
...
class LaunchChildrenWithTaskHandleDriver {
    tbb::internal::aligned_space<handle_type,NUM_CHORES> m_handles;
...
public:
    void Launch ( task_group_type& tg ) {
        ...
        ASSERT ( !tbb::is_current_task_group_canceling(), "Unexpected cancellation" );
...
void RunCancellationAndExceptionHandlingTests() {
    TestManualCancellationWithFunctor              <tbb::task_group>();
    TestManualCancellationWithTaskHandle<tbb::task_group>();
#if TBB_USE_EXCEPTIONS && !__TBB_THROW_ACROSS_MODULE_BOUNDARY_BROKEN
    TestExceptionHandling1<tbb::task_group>();
    TestExceptionHandling2<tbb::task_group>();

    TestMissingWait<tbb::task_group, true>();
#if !(__TBB_THROW_FROM_DTOR_BROKEN || __TBB_STD_UNCAUGHT_EXCEPTION_BROKEN)
    TestMissingWait<tbb::task_group, false>();
#else
...
#if __TBB_LEGACY_MODE
template<> void RunCancellationAndExceptionHandlingTests<tbb::structured_task_group>() {
    TestManualCancellationWithTaskHandle<tbb::structured_task_group>();
#if TBB_USE_EXCEPTIONS && !__TBB_THROW_ACROSS_MODULE_BOUNDARY_BROKEN
    TestMissingWait<tbb::structured_task_group, true>();
#if !(__TBB_THROW_FROM_DTOR_BROKEN || __TBB_STD_UNCAUGHT_EXCEPTION_BROKEN)
    TestMissingWait<tbb::structured_task_group, false>();
#else
    REPORT("Known issue: TestMissingWait<task_group_type, false>() is skipped.\n");
#endif
#endif /* TBB_USE_EXCEPTIONS && !__TBB_THROW_ACROSS_MODULE_BOUNDARY_BROKEN */
...
namespace TestIsolationNS {
    ...
    class ParForBody {
        task_group_type& m_tg;
        Harness::atomic<bool>& m_preserved;
        tbb::enumerable_thread_specific<int>& m_ets;
    public:
        ParForBody(
            task_group_type& tg,
            Harness::atomic<bool>& preserved,
    ...
    void CheckIsolation(bool isolation_is_expected) {
        task_group_type tg;
        Harness::atomic<bool> isolation_is_preserved;
...
int TestMain () {
    ...
    for( int p=MinThread; p<=MaxThread; ++p ) {
        g_MaxConcurrency = p;
#if TBBTEST_USE_TBB
        Harness::task_scheduler_init init(p);
#else
...
#if __TBB_LEGACY_MODE
        RunFibonacciTests<tbb::structured_task_group>();
#endif
        RunCancellationAndExceptionHandlingTests<tbb::task_group>();
#if __TBB_LEGACY_MODE
        RunCancellationAndExceptionHandlingTests<tbb::structured_task_group>();
#endif
#if TBBTEST_USE_TBB && TBB_PREVIEW_ISOLATED_TASK_GROUP
        if ( p > 1 ) {
