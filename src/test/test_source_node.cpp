...
template< typename T >
class test_push_receiver : public tbb::flow::interface11::receiver<T>, NoAssign {

    Harness::atomic<int> my_counters[N];
...
class function_body {
    ...
    Harness::atomic<int> *my_counters;

public:

    function_body( Harness::atomic<int> *counters ) : my_counters(counters) {
        for (int i = 0; i < N; ++i )
            my_counters[i] = 0;
    }
...
void test_single_dest() {
   ...
   // push only
   Harness::atomic<int> counters3[N];
   ...
   Harness::atomic<int> counters2[N];
