...
#include "harness_barrier.h" // for Harness::atomic
...
template< typename T >
struct serial_receiver : public tbb::flow::interface11::receiver<T>, NoAssign {
   T next_value;
...
template< typename T >
struct parallel_receiver : public tbb::flow::interface11::receiver<T>, NoAssign {

    Harness::atomic<int> my_count;
    tbb::flow::graph& my_graph;
...
template< typename T >
struct empty_sender : public tbb::flow::interface11::sender<T> {
...
struct put_body : NoAssign {
    ...
    Harness::atomic<int> &my_accept_count;

    put_body( tbb::flow::limiter_node<T> &lim, Harness::atomic<int> &accept_count ) :
...
struct put_dec_body : NoAssign {
    ...
    put_dec_body( tbb::flow::limiter_node<T> &lim, Harness::atomic<int> &accept_count ) :
        my_lim(lim), my_accept_count(accept_count) {}
...
void test_puts_with_decrements( int num_threads, tbb::flow::limiter_node< T >& lim , tbb::flow::graph& g) {
    ...
    Harness::atomic<int> accept_count;
    accept_count = 0;
...
Harness::atomic<size_t> emit_count;
Harness::atomic<size_t> emit_sum;
Harness::atomic<size_t> receive_count;
Harness::atomic<size_t> receive_sum;

struct mfnode_body {
    int max_cnt;
    Harness::atomic<int>* my_cnt;
    mfnode_body(const int& _max, Harness::atomic<int> &_my) : max_cnt(_max), my_cnt(&_my)  { }
...
void test_extract() {
    ...
    for( int i = 0; i < 2; ++i ) {
        ...
        tbb::flow::interface11::continue_receiver::predecessor_list_type dv;
        tbb::flow::limiter_node<int>::successor_list_type sv1;
        tbb::flow::limiter_node<int>::predecessor_list_type pv1;
        tbb::flow::interface11::continue_receiver::predecessor_list_type dv1;
...
int TestMain() {
    for (int i = 1; i <= 8; ++i) {
        Harness::task_scheduler_init init(i);
        test_serial<int>();
        test_parallel<int>(i);
    }
    test_continue_msg_reception();
